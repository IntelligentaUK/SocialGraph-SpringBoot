...
Rust HTTP sidecar that hosts the SigLIP-2 image+text encoder and a Gemma
vision-language model for the SocialGraph vector-search pipeline.

## Endpoints

| Method | Path                | Body                                    | Returns                       |
|-------:|:--------------------|:----------------------------------------|:------------------------------|
| `GET`  | `/healthz`          | —                                       | `200 {status:"ok"}` once both models loaded, `503 {status:"loading",...}` before |
...
| `GEMMA_MODEL_ID`           | `google/gemma-3-4b-it`                      | HF model repo for the Gemma VLM (spec referenced `gemma-4-E2B` which is unverified on HF Hub — this default is the closest shipped VLM; override as models ship) |
